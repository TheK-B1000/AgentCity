...
const Ajv = _Ajv;
import { createTrace, buildEvent } from "./traceLogger.js";
import { callTool, resetBudgets } from "./toolGateway.js";
 ...
 */
export async function runWorkflow(foundationRoot, workflowPath, cityConfig, options = {}) {
    // Initialize drivers
    await initDrivers();
    // Load workflow YAML
...
// ── Step Handlers ──────────────────────────────────────────────────────
async function handleStepStart(index, step, trace, runInfo, agent) {
...
async function handleStepEnd(index, step, trace, runInfo, agent) {
...
async function handleToolCall(index, step, trace, runInfo, agent, foundationRoot, cityConfig, options, traceDate) {
    console.log(`   → [${index}] tool_call: ${step.name} → ${step.tool}`);
    if (!step.tool)
        throw new Error(`Step "${step.name}" is tool_call but has no "tool" field.`);
    if (!step.building)
        throw new Error(`Step "${step.name}" has no "building" field.`);
    // Resolve building by environment stage pointer (not hardcoded)
    const envStage = cityConfig.environment;
    ...
    if (step.sop_path) {
        ...
        console.log(`     ├─ SOP loaded: ${sop?.sop ?? step.sop_path}`);
    }
...
async function handleVerify(index, step, trace, runInfo, agent, foundationRoot, output) {
...
async function handleReport(index, step, trace, runInfo, agent, runOutput, foundationRoot, traceId, traceDate, options) {
    console.log(`   → [${index}] report: ${step.name}`);
    const result = runOutput.result;
    const output = result?.payload;
    const summary = output
        ? `output_keys=${Object.keys(output).join(",")}`
        : "no output";
    ...
    const summaryMd = [
        ...
        `## Citations`,
        ``,
        ...(typedOutput?.citations ?? []).map((c) => `- **${c.source}** — ${c.locator}`),
        ``,
    ...
    const { merged, conflicts } = mergeMetadata(producerMeta, runMeta);
    ...
    const extRecords = merged.external_records;
...
function mergeMetadata(producer, run) {
    const conflicts = [];
    const merged = { ...run }; // Start with run as baseline
