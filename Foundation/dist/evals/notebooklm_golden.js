...
export async function run(foundationRoot, driver) {
    const checks = [];
    ...
    checks.push({
        ...
        pass: !!schemaValid,
        detail: schemaValid ? "Output matches output.schema.json" : `Schema errors: ${validate.errors?.map((e) => e.message).join("; ")}`,
    });
    ...
    if (traceExists) {
        ...
        try {
            traceLines.forEach((l) => JSON.parse(l));
...
//# sourceMappingURL=notebooklm_golden.js.map