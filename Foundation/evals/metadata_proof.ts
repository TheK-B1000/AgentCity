 ...
 */
export async function run(foundationRoot: string, driverName: string) {
...
async function runProofWorkflow(root: string, driverName: string, question: string) {
...
async function testA_ContractValidation(root: string, driverName: string) {
    ...
    if (!existsSync(metaPath)) throw new Error(`Missing metadata file: ${metaPath}`);

    const meta = JSON.parse(readFileSync(metaPath, "utf-8"));

    // 2. Assert meta-owned fields
    if (meta.driver !== driverName) throw new Error(`Metadata mismatch: driver=${meta.driver}`);
    // notebook.title might differ in mock vs autopilot, but key must exist
    if (!meta.notebook || !("title" in meta.notebook)) throw new Error("Missing notebook.title in metadata");

    // 3. Assert validated artifacts/output.json is payload-only
    const outputPath = resolve(artifactsDir, "output.json");
    const output = JSON.parse(readFileSync(outputPath, "utf-8"));

    if ("meta" in output) throw new Error("output.json contains 'meta' field (leaked)");
    if ("_driverMeta" in output) throw new Error("output.json contains '_driverMeta' field (legacy leak)");
    if ("artifacts" in output) throw new Error("output.json contains 'artifacts' wrapper field");

    if (!output.answer || !output.citations) throw new Error("output.json missing required payload fields");

    console.log("   ✓ Metadata separated from payload");
}

async function testB_MergePrecedence(root: string, driverName: string) {
    ...
    if (!meta.notebook) throw new Error("Missing producer-owned 'notebook' field");
    if (!meta.driver) throw new Error("Missing producer-owned 'driver' field");

    // 2. Assert Run-owned fields (from workflow)
    if (!meta.links) throw new Error("Missing run-owned 'links' field");
    if (!meta.building_version) throw new Error("Missing run-owned 'building_version'");

    // 3. Conflict Check
    if (!Array.isArray(meta.conflicts)) throw new Error("Missing 'conflicts' array in metadata");

    console.log("   ✓ Merge precedence respected");
}

async function testC_StructuredNulls(root: string, driverName: string) {
    ...
    if (meta.external_records) {
        for (const [key, val] of Object.entries(meta.external_records)) {
            if (val === null) throw new Error(`Field external_records.${key} is bare null. Must be structured object or omitted.`);

            if (typeof val === 'object' && val !== null && (val as any).status === "missing") {
                if (!("path" in (val as any)) || (val as any).path !== null) {
