...
import type { Driver, ToolInput, ToolOutput, ToolResult, ToolMeta, ArtifactLink, DriverContext } from "./index.js";
...
export const autopilotDriver: Driver = {
    ...
    async execute(input: ToolInput, ctx: DriverContext): Promise<ToolResult<ToolOutput>> {
...
interface ExternalRecordsMap {
    [key: string]: ExternalRecordEntry;
}
 ...
 */
function copyExternalBriefs(ctx: DriverContext, artifactsDir: string): ExternalRecordsMap {
    const docsRoot = resolve(ctx.foundationRoot, "..", "VerseRidge Corporate", ".agent", "docs");
    const externalDir = resolve(artifactsDir, "external_records");
    mkdirSync(externalDir, { recursive: true });

    const records: ExternalRecordsMap = {};
